...
CVE-2025-62215 là một lỗ hổng **Elevation of Privilege (EoP)** (nâng cao đặc quyền) trong Windows Kernel. Điều này có nghĩa là kẻ tấn công có thể lợi dụng lỗ hổng này để đạt được các quyền cao hơn trên hệ thống đã bị xâm nhập, chẳng hạn như truy cập cấp hệ thống. Từ đó, chúng có thể thực thi mã tùy ý hoặc truy cập dữ liệu vượt quá quyền hạn hiện có [2][3][4]. Các lỗ hổng Windows Kernel EoP thường phát sinh từ việc xử lý không đúng cách các đối tượng, quyền hoặc bộ nhớ, cho phép kẻ tấn công thực thi mã với các đặc quyền của kernel (SYSTEM) [Perplexity: A Windows Kernel Elevation of Privilege vulnerability is a security flaw in the Windows operating system's kernel that allows an attacker or a user with limited privileges to gain elevated (higher, often administrative or SYSTEM-level) access by exploiting specific bugs or weaknesses].

Với mức độ nghiêm trọng được đánh giá cao (CVSS 7.5) và đặc biệt là việc nó đang bị khai thác tích cực, lỗ hổng này yêu cầu các tổ chức và người dùng cá nhân phải cập nhật hệ thống Windows của mình ngay lập tức để giảm thiểu rủi ro [3][7][8]. Các lỗ hổng EoP trong kernel Windows đang bị khai thác thường có điểm CVSS từ 7.0 đến 7.8 [Perplexity: The typical CVSS score range for actively exploited Windows Kernel Elevation of Privilege vulnerabilities is 7.0 to 7.8, with some cases reaching slightly higher but rarely exceeding 8.8].
