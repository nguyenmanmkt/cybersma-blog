...
Tháng 1 năm 2026 đánh dấu một khởi đầu đầy thách thức cho các chuyên gia an ninh mạng với bản vá Patch Tuesday của Microsoft. Bản cập nhật lần này không chỉ khắc phục số lượng lớn các lỗ hổng mà còn đặc biệt cảnh báo về ba lỗ hổng zero-day, trong đó có một lỗ hổng đang bị tin tặc khai thác tích cực trong thực tế. Điều này nhấn mạnh sự cấp bách của việc duy trì các bản vá bảo mật và sự cảnh giác liên tục trước các mối đe dọa mới.
...
Bản vá tháng 1/2026 của Microsoft đã khắc phục tổng cộng từ **112 đến 115 lỗ hổng** bảo mật mới [1, 2, 4, 5]. Trong số này, có **8 lỗ hổng được đánh giá là Critical (nghiêm trọng)**, và phần lớn còn lại là Important (quan trọng) [1, 2].
...
*   **Tình trạng khai thác**: Đáng báo động là lỗ hổng này đã bị **khai thác tích cực trong thực tế** [1, 3]. Điều này có nghĩa là các hệ thống chưa được vá có nguy cơ rất cao bị tấn công. Lỗ hổng này cũng đã được thêm vào danh mục các lỗ hổng đã bị khai thác của CISA (KEV Catalog) [6].
*   **Sản phẩm bị ảnh hưởng**: Rất nhiều phiên bản của hệ điều hành Windows bị ảnh hưởng, bao gồm các phiên bản Windows 10, Windows 11 và các phiên bản Windows Server từ 2012 đến 2025 [3].
...
*   **CVE-2026-21265 (Windows Secure Boot)**: Đây là một lỗ hổng bỏ qua tính năng bảo mật khi hết hạn chứng chỉ Secure Boot (Secure Boot Certificate Expiration Security Feature Bypass) với CVSS 6.4 (hoặc 5.6) [10]. Lỗ hổng này xuất phát từ việc các chứng chỉ Secure Boot ban đầu của Microsoft sắp hết hạn, có thể làm gián đoạn chuỗi tin cậy của Secure Boot nếu không được cập nhật. Lỗ hổng này đã được công khai nhưng không có thông tin về việc bị khai thác tích cực [10].
*   **CVE-2026-20876 (Windows VBS Enclave)**: Đây là một lỗ hổng leo thang đặc quyền (Elevation of Privilege) loại use-after-free trong Windows VBS Enclave, được đánh giá là Critical. Lỗ hổng này cũng đã được công khai nhưng không bị khai thác tích cực [2].
...
*   **Microsoft Office**: Các lỗ hổng thực thi mã từ xa (RCE) tiềm ẩn. Đáng chú ý là các lỗi RCE nghiêm trọng trong Microsoft Office (CVE-2026-20952 và CVE-2026-20953) với CVSS 8.4, có thể bị khai thác thông qua các tài liệu độc hại [6, 9].
...
Với sự xuất hiện của một lỗ hổng zero-day đang bị khai thác, việc cập nhật bảo mật trở nên cấp thiết hơn bao giờ hết.
...
Microsoft Patch Tuesday tháng 1/2026 đã đưa ra một lời cảnh báo rõ ràng về bối cảnh mối đe dọa đang phát triển. Sự tồn tại của các lỗ hổng zero-day, đặc biệt là CVE-2026-20805 đang bị khai thác tích cực, nhấn mạnh tầm quan trọng của việc cập nhật bảo mật liên tục và chủ động. Bằng cách ưu tiên vá lỗi và áp dụng các biện pháp an ninh mạng mạnh mẽ, các tổ chức có thể bảo vệ hệ thống của mình trước những rủi ro ngày càng tăng này.
